...
import com.gkfcsolution.springboot_commons_taks_ch2.repository.PageableCourseRepository;
...
@DataJpaTest
public class CrudPaginationResquestTest {
    @Autowired
    private PageableCourseRepository pageableCourseRepository;
    ...
    public void givenDataAvailableWhenLoadFirstPageThenGetFiveRecords() {
        ...
        assertThat(pageableCourseRepository.findAll(pageable)).hasSize(5);
        assertThat(pageable.getPageNumber()).isEqualTo(0);

        Pageable nextPageable = pageable.next();
        assertThat(pageableCourseRepository.findAll(nextPageable)).hasSize(2);
    ...
    public void givenDataAvailableWhenSortsFirstPageThenGetSortedSData() {
        ...
        Condition<Course> sortedFirstCourseCondition = new Condition<>() {
            ...
            public boolean matches(Course course) {
                return course.getId() == 5 && course.getName().equalsIgnoreCase("Game Development with Python");
            }
        };
        assertThat(pageableCourseRepository.findAll(pageable)).first().has(sortedFirstCourseCondition);
    }
    ...
    public void givenDataAvailableWhenApplyCustomSortThenGetSortedResult() {
        ...
        assertThat(pageableCourseRepository.findAll(customSortPageable)).first().has(customSortFirstCourseCondition);
    }
}
